...
combnd <- prcp |> 
    summarize(.by = c(BAY_SEG, month, year),
              inches = mean(inches, na.rm = TRUE)) |> 
    filter(year >= 2009,
           BAY_SEG %in% unique(hydro$bay_segment)) |>
    left_join(hydro, by = c("BAY_SEG" = "bay_segment", "year", "month")) |> 
    rename(bay_segment = BAY_SEG,
           hydro_load = hy_load_106_m3_mo)
```
...
fib_hydro <- left_join(fib_baysegs, hydro_sub,
                       by = c("yr" = "year",
                              "mo" = "month",
                              "bay_segment")) |> 
